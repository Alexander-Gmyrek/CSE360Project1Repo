...
//UML DESCRIPTION
// In our design we envisioned a user management system, that would store users and allow for operations to be completed on their
// accounts so they could be used in the system. This design flowed to the code as this code allows the system to store the current
// users, check for a first user (to make them admin), allows for the creation of new accounts so users can use the system, and allows
// logins so old users can use the system as well. Users can also have one time passwords and invitation codes generated for them
// which implements the design further by allowing for new users to be created and current users to ensure they can sign back in
// if they forget their account details. Furthermore, our role management service that we wanted where users could be assigned roles
// in the system was implemented here as well as accounts on creation have their roles updated here and roles can be updated
